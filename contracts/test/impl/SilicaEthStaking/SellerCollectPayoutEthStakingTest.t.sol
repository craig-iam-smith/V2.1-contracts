...
contract SellerCollectPayoutEthStaking is BaseTest {
    ...
    function testSellerCollectPayoutWithExcessWhenBuyerDepositIsVerySmallAndInitialCollateralCoversTheContractWithExcess() public {
        ...
        uint256 buyerDeposit = 24;
        silicaEthStaking.setResourceAmount(defaultStakingAmount);
        silicaEthStaking.setReservedPrice(defaultStakingReservedPrice * 100000);
        silicaEthStaking.setInitialCollateral(defaultStakingInitialCollateral);
        ...
        uint256 contractBalanceBeforeCP = stakingRewardToken.balanceOf(address(silicaEthStaking));
        uint256 rewardDueWhenFinished = TestHelpers.getTotalRewardDueWhenFinishedEthStaking(
            address(silicaEthStaking),
            address(oracleEthStaking),
            40
        );
        uint256 expectedExcess = contractBalanceBeforeCP - rewardDueWhenFinished;
        uint256 sellerRewardBalanceBeforeCP = stakingRewardToken.balanceOf(sellerAddress);

        cheats.prank(sellerAddress);
        silicaEthStaking.sellerCollectPayout();

        assertEq(silicaEthStaking.rewardExcess(), expectedExcess);
        assertEq(stakingRewardToken.balanceOf(sellerAddress), sellerRewardBalanceBeforeCP + expectedExcess);
        assertEq(paymentToken.balanceOf(sellerAddress), buyerDeposit);
        assertEq(silicaEthStaking.didSellerCollectPayout(), true);
    }

    function testSellerCollectPayoutWithoutExcess() public {
        //BUYER DEPOSITS
        uint256 reservedPrice = silicaEthStaking.getReservedPrice();
    ...
    function testSellerCollectPayout() public {
        ...
        silicaEthStaking.setFinishDay(1);
        silicaEthStaking.setRewardExcess(rewardExcess);

        cheats.startPrank(address(silicaEthStaking));
        stakingRewardToken.getFaucet(rewardExcess + initialCollateral);
        ...
        cheats.prank(sellerAddress);
        silicaEthStaking.sellerCollectPayout();
        assertEq(stakingRewardToken.balanceOf(sellerAddress), sellerRewardBalanceBeforeCP + rewardExcess);
        assertEq(paymentToken.balanceOf(sellerAddress), totalUpfrontPayment);
        assertEq(stakingRewardToken.balanceOf(address(silicaEthStaking)), initialCollateral);
    ...
    function testSellerCollectPayoutTwice() public {
        ...
        cheats.startPrank(address(silicaEthStaking));
        stakingRewardToken.getFaucet(initialCollateral);
        paymentToken.getFaucet(totalUpfrontPayment);
        cheats.stopPrank();
