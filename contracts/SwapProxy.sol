 ...
 */
contract SwapProxy is EIP712, ISwapProxy, Pausable {
    ...
    ) external whileNotPaused returns (address) {
    ...
    function cancelBuyOrder(OrderLib.BuyOrder calldata order, bytes memory signature) external whileNotPaused {
     ...
     * @param signature The signature of the signer 
     */
    function cancelSellOrder(OrderLib.SellOrder calldata order, bytes memory signature) external whileNotPaused {
